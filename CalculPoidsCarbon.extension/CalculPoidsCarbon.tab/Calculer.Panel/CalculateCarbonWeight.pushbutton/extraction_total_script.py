...
from Autodesk.Revit.DB import BuiltInParameter, FilteredElementCollector, Phase, Transaction, ParameterType, BuiltInParameterGroup, ProjectInfo, BuiltInCategory, ExternalDefinitionCreationOptions, ElementId
clr.AddReference('System.Windows.Forms')
import System
from System.Drawing import Size
from System.Windows.Forms import Form, FormStartPosition, CheckBox, ComboBox, Button, DialogResult, MessageBox , MessageBoxButtons, MessageBoxIcon
import csv
import psycopg2
import sqlalchemy
...
filtered_list = [a for a in has_material_volume if a is not None and a.Category is not None and a.Category.Name != "Garde-corps" and a.Category.Name != "Barreaux"]
...
if project_id:
    class SimpleForm(Form):
        def __init__(self):
            ...
            self.StartPosition = FormStartPosition.CenterScreen  # Center the form on the screen
            self.Size = Size(400, 375)
            
            self.local_calculation_checkbox = CheckBox()
            self.local_calculation_checkbox.Text = 'Faire un calcul local (pas envoyé au Calculette Carbon)'
        ...
        def validate_button_clicked(self, sender, args):
            ...
            selected_options = [self.new_version_checkbox.Checked, self.update_version_checkbox.Checked, self.local_calculation_checkbox.Checked, self.final_phase_calcul_checkbox.Checked]
    
            # Ensure that only one option is selected
            if sum(selected_options) != 1:
                MessageBox.Show("Veuillez cocher une case.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            elif self.update_version_checkbox.Checked and not self.version_dropdown.SelectedItem:
                MessageBox.Show("Veuillez choisir une version", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
            else:
    ...
    def show_dialog():
        ...
        if result == DialogResult.OK:
            ...
            selected_version = form.version_dropdown.SelectedItem if update_version else None
...
if project_id is None:
    ...
    req_params = {**common_params, 'user': username, 'persist': True, 'project_id': project_id}
elif project_id and update_version:
    req_params = {**common_params, 'user': username, 'persist': True, 'project_id': project_id, 'version_id': version_id}
...
if req_params:    
    ...
    def create_project_parameter(doc):
        ...
        doc.ParameterBindings.Insert(definition, instanceBinding, BuiltInParameterGroup.PG_DATA)
    
        # Commit the transaction
        t.Commit()
        return "Parameter created successfully."
    
    row_list = [["Version name", "Version ID"],
                ["Version 1", results["version_id"]]]
    ...
    if not "Project_Id" in proj_info_param_names:
        ...
        project_info = collector.OfCategory(BuiltInCategory.OST_ProjectInformation).WhereElementIsNotElementType().ToElements()
        ...
        for info in project_info:
            ...
            if project_shared_param:
                ...
                project_shared_param.Set(results["project_id"])  # Set the value to 10.0 for example
    ...
    for key, value in element_quantity_dict.items():
        ...
        try:
            ...
            for sous_projet in sous_projet_data:
                if sous_projet_name in sous_projet.keys():
                    ...
                    if mat_class not in sous_projet[sous_projet_name]["material_class_calculated"]:
                        sous_projet[sous_projet_name]["material_class_calculated"].append(mat_class)
                    if type_name not in sous_projet[sous_projet_name]["type_calculated"]:
                        sous_projet[sous_projet_name]["type_calculated"].append(type_name)
                    if family_name not in sous_projet[sous_projet_name]["family_calculated"]:
                        sous_projet[sous_projet_name]["family_calculated"].append(family_name)
                    
                    # Update material_classes_calculated with the count of material classes
                    sous_projet[sous_projet_name]["material_class_count"] = len(set(sous_projet[sous_projet_name]["material_class_calculated"]))
                    sous_projet[sous_projet_name]["type_count"] = len(set(sous_projet[sous_projet_name]["type_calculated"]))
                    sous_projet[sous_projet_name]["family_count"] = len(set(sous_projet[sous_projet_name]["family_calculated"]))
        ...
        try:
            ...
            for element_category in family_data:
                if family_name in element_category.keys():
                    ...
                    if mat_class not in element_category[family_name]["material_class_calculated"]:
                        element_category[family_name]["material_class_calculated"].append(mat_class)
                    if type_name not in element_category[family_name]["type_calculated"]:
                        element_category[family_name]["type_calculated"].append(type_name)
                        
                    # Update material_classes_calculated with the count of material classes
                    element_category[family_name]["material_class_count"] = len(set(element_category[family_name]["material_class_calculated"]))
                    element_category[family_name]["type_count"] = len(set(element_category[family_name]["type_calculated"]))
        except:
            pass          
        ...
        try:
            ...
            for type in type_data:
                if type_name in type.keys():
                    ...
                    # Update material_classes_calculated with the count of material classes
                    type[type_name]["material_class_count"] = len(set(type[type_name]["material_class_calculated"]))
        except:
            pass
        multiple_materials = ""
        try:
        ...
        for material in materials:
            elem_mat = doc.GetElement(material)
            mat_class = elem_mat.MaterialClass
            if all(mat_class not in d.keys() for d in material_class_data) and mat_class not in materiaux_inconnus:
    ...
    phase_data = (phase_nom, total.replace(',', ''), date.today(), elements_calculated_for_phase, material_classes_for_phase, family_data_for_phase, type_data_for_phase)   
    ...
    if final_phase_calcul:
        ...
        Project.phase = relationship("Phase", order_by=Phase.phase_id, back_populates="project")
        ...
        Project.sous_projet = relationship("SousProjet", order_by=SousProjet.sous_projet_id, back_populates="project")
        Phase.sous_projet = relationship("SousProjet", order_by=SousProjet.sous_projet_id, back_populates="phase")
        ...
        Project.family = relationship("Family", order_by=Family.family_id, back_populates="project")
        Phase.family = relationship("Family", order_by=Family.family_id, back_populates="phase")        
        ...
        # Add the material_classes relationship to the Project, Phase, and Sous_projet classes
        Project.type = relationship("Type", order_by=Type.type_id, back_populates="project")
        ...
        # Add the material_classes relationship to the Project, Phase, and Sous_projet classes
        Project.instance = relationship("Instance", order_by=Instance.instance_id, back_populates="project")
        Phase.instance = relationship("Instance", order_by=Instance.instance_id, back_populates="phase")
        ...
        # Add the material_classes relationship to the Project, Phase, and Sous_projet classes
        Project.material_class = relationship("MaterialClass", order_by=MaterialClass.material_class_id, back_populates="project")
        Phase.material_class = relationship("MaterialClass", order_by=MaterialClass.material_class_id, back_populates="phase")

        def insert_data_into_database(project_data, phase_data, sous_projet_data, family_data, type_data, instance_data, material_class_data):
            ...
            for instance_entry, sous_projet_entry, family_entry, type_entry, material_class_entry in zip_longest(instance_data, sous_projet_data, family_data, type_data, material_class_data, fillvalue=None):
                ...
                if sous_projet_entry is not None:
                    sous_projet = SousProjet(
                        ...
                        carbon_weight_kg=sous_projet_entry[sous_projet_name]["carbon_weight"],
                        instance_calculated=sous_projet_entry[sous_projet_name]["instance_calculated"],
                        material_class_calculated=sous_projet_entry[sous_projet_name]["material_class_count"],
                        family_calculated=sous_projet_entry[sous_projet_name]["family_count"],
                        type_calculated=sous_projet_entry[sous_projet_name]["type_count"],
                        phase=phase,
                        project=project
                    )
                ...
                if family_entry is not None:
                    family = Family(
                        ...
                        instance_calculated=family_entry[family_name]["instance_calculated"],
                        material_class_calculated=family_entry[family_name]["material_class_count"],
                ...
                if type_entry is not None:
                    type = Type(
                        ...
                        instance_calculated=type_entry[type_name]["instance_calculated"],
                        material_class_calculated=type_entry[type_name]["material_class_count"],
                ...
                if material_class_entry is not None:
                    material_class = MaterialClass(
                        ...
                        carbon_weight_kg=material_class_entry[material_class_name]["carbon_weight"],
                        instance_calculated=material_class_entry[material_class_name]["instance_calculated"],
                        phase=phase,
                        project=project,
                    )
        ...
        insert_data_into_database(project_data, phase_data, sous_projet_data, family_data, type_data, instance_data, material_class_data)
    alert = TaskDialog.Show("Calcul réussi", "L'extraction et le processus de calcul ont réussi. Veuillez importer les calculs dans les éléments du projet ou consultez les résultats dans le fichier 'carbon_data' au format html enregistré dans un dossier appelé 'carbon_data' sur votre bureau.")
else:
