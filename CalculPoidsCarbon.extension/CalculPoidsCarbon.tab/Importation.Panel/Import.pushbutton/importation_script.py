...
SetInstanceParamVaryBetweenGroupsBehaviour(
    doc, poids_carbone_guid, allowVaryBetweenGroups=True
)
        ...
        if dico["quantity"] == result["Quantite"]:
            for i in range(len(dico["individual_quantities"])):
                ...
                element_carbon_weight = (
                    r * result["Impact sur le changement climatique (kgCO2e)"]
                )
...
filter_names = [
    "C_05_TRES_GRAND_POIDS_CARBON",
    "C_04_GRAND_POIDS_CARBON",
    "C_03_MOYEN_POIDS_CARBON",
    "C_02_PETIT_POIDS_CARBON",
    "C_01_TRES_PETIT_POIDS_CARBON",
]

# Delete existing filters
transaction_delete = Transaction(doc, "Delete Filters")
transaction_delete.Start()

for existing_filter in existing_filters:
    if existing_filter.Name in filter_names:
        doc.Delete(existing_filter.Id)
...
for elem in has_material_volume:
    if elem is not None and elem.LookupParameter("_POIDS_CARBONE") is not None:
        ...
        very_large_rule = ParameterFilterRuleFactory.CreateGreaterRule(
            parameter_id, very_large_value, False
        )
        large_rule = ParameterFilterRuleFactory.CreateGreaterRule(
            parameter_id, large_value, False
        )
        medium_rule = ParameterFilterRuleFactory.CreateGreaterRule(
            parameter_id, medium_value, False
        )
        small_rule = ParameterFilterRuleFactory.CreateGreaterRule(
            parameter_id, small_value, False
        )
        very_small_rule = ParameterFilterRuleFactory.CreateGreaterRule(
            parameter_id, very_small_value, False
        )

        transaction = Transaction(doc, "Create Filters")
        transaction.Start()
        # Create each filter for different sizes and apply the correct rule
        very_large_filter = ParameterFilterElement.Create(
            doc, "C_05_TRES_GRAND_POIDS_CARBON", material_volume_collection_net
        )
        apply_very_large_rule = ElementParameterFilter(very_large_rule)
        very_large_filter.SetElementFilter(apply_very_large_rule)

        large_filter = ParameterFilterElement.Create(
            doc, "C_04_GRAND_POIDS_CARBON", material_volume_collection_net
        )
        apply_large_rule = ElementParameterFilter(large_rule)
        large_filter.SetElementFilter(apply_large_rule)

        medium_filter = ParameterFilterElement.Create(
            doc, "C_03_MOYEN_POIDS_CARBON", material_volume_collection_net
        )
        apply_medium_rule = ElementParameterFilter(medium_rule)
        medium_filter.SetElementFilter(apply_medium_rule)

        small_filter = ParameterFilterElement.Create(
            doc, "C_02_PETIT_POIDS_CARBON", material_volume_collection_net
        )
        apply_small_rule = ElementParameterFilter(small_rule)
        small_filter.SetElementFilter(apply_small_rule)

        very_small_filter = ParameterFilterElement.Create(
            doc, "C_01_TRES_PETIT_POIDS_CARBON", material_volume_collection_net
        )
...
phase_selected = forms.SelectFromList.show(
    phase_filter_names, button_name="Choissisez le filtre", multiselect=False
)
view_templates = (
...
if phase_selected:
    transaction_delete = Transaction(doc, "Delete VT")
    transaction_delete.Start()
    for temp in view_templates:
        if "CLC_CARBON_" in temp.Name:
            doc.Delete(temp.Id)
        ...
        if has_id:
            ...
            if element_id_str in element_quantity_dict:
                ...
                try:
                    ...
                    print(
                        "Erreur lors de la mise à jour de l'élément {element.Id} : {str(e)}. Veuillez réessayer ultérieurement."
                    )
    t.Commit()
    ...
    if default_3d_view:
        ...
        with Transaction(doc, "Activate template") as transaction:
            transaction.Start()
            template_name = (
                "CLC_CARBON_" + phase_selected.upper()
            )  # Replace with the name of your view template
    ...
    TaskDialog.Show(
        "Filter de phase manquant",
        "Veuillez choisir un filtre de phase pour completer l'importation des valeurs.",
    )
