...
acier = []
laines_minerales = []
autres_materiaux = ["Pas d'attribution", "Lot 11", "Lot 09", "Lot 07", "Lot 04", "Générique", "Divers", "Textile", "System", "Système", "Generic", "Miscellaneous", "Non attribuée", "Unassigned", "Genérico", "Sin asignar", "Sistema", "Varios"]
aluminium = []
peinture = ["Peinture/revêtement", "Paint/Coating", "Peindre", "Peinture"]
autres_materiaux_biosources = ["Céramique", "Gaz", "Liquide", "Terre", "Ceramic", "Gas", "Plante", "Cerámica", "Soil", "Terreno"]
...
for item in filtered_elements:
    ...
    if len(materials) == 0 and item.Category.Name in no_material_check and hasattr(volume, "AsDouble"):
...
if len(no_material) > 0:
    for dict in no_material:
        updated_dict = {}  # Create a new dictionary to store the updated key-value pairs
        for key, value in dict.items():
            new_value = value.replace("\xe9", "e")
            updated_dict[key] = new_value  # Store the updated key-value pair in the new dictionary
        updated_material.append(updated_dict)  # Append the updated dictionary to the new list
    ...
    TaskDialog.Show("Aucun matériau manquant trouvé", "Vous n'avez aucun élément sans matériaux")